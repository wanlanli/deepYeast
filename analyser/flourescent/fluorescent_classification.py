...
#         self.channel_number = self.image.shape[0] - 1
#         self.fluorescent_intensity = None
#         self.classifier = None

#     def __flatten_nonzero_value(self, data):
...
#                 chi = self.__flatten_nonzero_value(cell_mask*self.image[i])
...
class FluorescentClassification():
    ...
    def normalize_inensity(self):
        for i in range(1, self.channel_number+1):
            self.data['ch%d_norm' % i] = (np.log(self.data['ch_%d' % i])-np.log(self.data['bg_%d' %i ])+1)/np.log(self.data['bg_%d' %i ])
        return self.data
