...
#   elif 'cellfustion_panoptic' in dataset_name:
#     train_id_to_eval_id = _CITYSCAPES_TRAIN_ID_TO_EVAL_ID
#   else:
#     raise ValueError(
...
def store_raw_predictions(predictions: Dict[str, Any],
                          ...
                          save_dir: Text,
                          sequence: tf.Tensor,
                          raw_panoptic_format='three_channel_png',
                          convert_to_eval=False):
...
def store_predictions(predictions: Dict[str, Any], inputs: Dict[str, Any],
  ...
  print("!Save image", save_dir, image.shape)
  imsave(os.path.join(save_dir, str(_IMAGE_FORMAT % image_id)+'.png'), image)
#   vis_utils.save_annotation(
#       image,
#       save_dir,
