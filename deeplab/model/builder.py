...
def create_encoder(backbone_options: config_yml.BackboneOptions,
  ...
  Args:
    backbone_options: A config of type
      config_yml.ModelOptions.BackboneOptions.
  ...
  Raises:
    ValueError: An error occurs when the specified encoder meta architecture is
      not supported.
  """
  if ('swidernet' in backbone_options.name or
...
def create_resnet_encoder(
    backbone_options: config_yml.BackboneOptions,
    bn_layer: tf.keras.layers.Layer,
    conv_kernel_weight_decay: float = 0.0) -> tf.keras.Model:
...
def create_decoder(model_options: config_yml.ModelOptions,
                   bn_layer: tf.keras.layers.Layer,
                   ) -> tf.keras.Model:
  """Creates a DeepLab decoder.

  Args:
    model_options: A proto config of type config_yml.ModelOptions.
    bn_layer: A tf.keras.layers.Layer that computes the normalization.
  ...
  Raises:
    ValueError: An error occurs when the specified meta architecture is not
      supported.
  """
  meta_architecture = model_options.meta_architecture
  if meta_architecture == 'panoptic_deeplab':
