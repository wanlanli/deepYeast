...
from .config import TRACING_FEATRUE_NAMES, PROP_NAMES
...
def frame_name(number, name='frame_', length=3):
...
class Tracer(object):
    def __init__(self, data) -> None:
        ...
        self.cells = []
        self.props = None
        self.__flag = False  # is traced
        self.tracingdata = None

    def __init_tracing_feature_data(self):
        frame_columns_name = [frame_name(x) for x in np.arange(0, self.frame_number)]
    ...
    def tracing(self, **args):
        ...
        for frame in tqdm(range(0, self.frame_number), position=0):
            ...
            dets = img.region.iloc[:, list(range(1, 6))+[0]].values
            total_frames += 1
            trackers = mot_tracker.update(np.array(dets))
            for d in trackers:
                output_d.append([frame]+list(d))
        output_d = np.array(output_d)  # [frame, x,y,r,l,s,id,label]
    ...
    def asgine_feature(self, output_d):
        for i in range(output_d.shape[0]):
            ...
            self.trace_calendar.loc[new_id, frame_name(frame)] = label
    ...
    def plot_tracing(self):
        # if not self.__flag:
        #     print("Please runing cell.tracing first")
        #     return self.mask
        tracing_data = np.zeros(self.mask.shape)
        for frame in range(0, self.frame_number):
            d = self.trace_calendar.loc[:, frame_name(frame)].dropna()
            for k, v in d.items():
