...
from model.decoder import aspp
from model.layers import convolutions
...
class PanopticDeepLab(layers.Layer):
  ...
  def __init__(self,
    ...
    low_level_feature_keys = [
        item for item in panoptic_deeplab_options.low_level.feature_key
    ]
    low_level_channels_project = [
        item for item in panoptic_deeplab_options.low_level.channels_project
    ]
    ...
    if panoptic_deeplab_options.instance.enable:
      ...
      if hasattr(panoptic_deeplab_options.instance, 'instance_decoder_override'):
        decoder_options = (panoptic_deeplab_options.instance
                           .instance_decoder_override)

      low_level_feature_keys = [item for item in low_level_options.feature_key]
      low_level_channels_project = [
          item for item in low_level_options.channels_project
      ]
